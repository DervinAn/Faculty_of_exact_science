...
private fun PlaceholderEventCard(
    ...
    EventCard(
        event = com.example.facultyofexactscience.faculty.domain.Event(
            title = "Event Title",
            description = "Lorem ipsum dolor sit amet, conslit.",
            date = "2025-02-01",
            time = com.example.facultyofexactscience.faculty.domain.Time("00:00","00:00"),
            isCurrent = false
        ),
...
fun EventCarousel(
    ...
    LazyRow(
        ...
        val padded = events + List((visible - (events.size % visible)) % visible) { events.last() }
        itemsIndexed(padded) { index, ev ->
            val idxInRow = index % visible
            EventCard(
                event = ev,
